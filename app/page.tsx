    ...
    try {
      const response = await fetch("/api/ai/generate-tasks", {
        ...
        body: JSON.stringify({
          ...
          milestoneTitle: selectedMilestoneId ? milestones.find(m => m.id === selectedMilestoneId)?.title : undefined,
                  ...
                  <div className="grid gap-3 md:grid-cols-[2fr_1fr]">
                    <div className="flex flex-col gap-3">
                      <div className="flex flex-col gap-2">
                        ...
                        <select
                          className="w-full rounded-2xl border border-transparent bg-[var(--panel)] px-4 py-3 text-sm shadow-[0_0_0_1px_rgba(15,23,42,0.1)] focus:outline-none focus:ring-2 focus:ring-[var(--ring)]"
                    ...
                    <div className="grid gap-3">
                      ...
                      <input
                        ...
                        value={budget || ""}
                        onFocus={(e) => e.target.select()}
                        onChange={(event) => handlePlanBudgetChange(event.target.value === "" ? 0 : Number(event.target.value))}
                        className="rounded-2xl border border-transparent bg-[var(--panel)] px-4 py-3 shadow-[0_0_0_1px_rgba(15,23,42,0.1)] focus:outline-none focus:ring-2 focus:ring-[var(--ring)]"
                      />
                      <button
                          ...
                          <DictationMic
                            ...
                            onClick={() => {
                              if (activeRecordingField === "manualTitle") stopRecording();
                                ...
                                const msContext = selectedMilestoneId ? ` Milestone: ${milestones.find(m => m.id === selectedMilestoneId)?.title}` : "";
